...
class ClientTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testCreateRds()
    {
        ...
        $response = $client->createRds('testConnection', 'testTable', $data);
        $this->assertInstanceOf('\Emarsys\Response', $response);
    }
     ...
     * @throws Exception\ServerException
     */
    public function testGetRds()
    {
        $expectedResponse = $this->createExpectedResponse('getRecord');
        $client           = $this->getGuzzleMock(200, $expectedResponse);

        $response = $client->getRds('testConnection', 'testTable', 'customerId', 123456);
        $this->assertInstanceOf('\Emarsys\Response', $response);
        $this->assertSame(Response::REPLY_CODE_OK, $response->getReplyCode());
        $this->assertSame('OK', $response->getReplyText());
     ...
     * @throws Exception\ServerException
     */
    public function testUpdateRds()
    {
        $expectedResponse = $this->createExpectedResponse('updateRecord');
        $client           = $this->getGuzzleMock(200, $expectedResponse);
        $requestData      = [
            ['search' => ['customer' => 123456], 'update' => ['street' => 'Teststreet']],
            ['search' => ['customer' => 987654], 'update' => ['street' => 'Unitstreet']]
        ];
        $response = $client->updateRds('testConnection', 'testTable', $requestData);
        $this->assertInstanceOf('\Emarsys\Response', $response);
        $this->assertSame(Response::REPLY_CODE_OK, $response->getReplyCode());
